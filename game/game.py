...
class Game:
    ...
    def loop(self):
        ...
        self.__put_text(f"Score: {self.player.score}", Vector2D(64 + 16, 0))
        self.__put_text(f"Health: {self.player.health}", Vector2D(64 + 16, 32))
        self.__put_text(f"Time: {self.GAME_TIME - self.tick}", Vector2D(64 + 16, 64))
    ...
    def __spawn_monter(self):
        if self.tick % self.MONSTER_TICKS == 0:
            ...
            while True:
                x = random.randint(64, Game.WIDTH - 64 - monster.rb.rect.width)
